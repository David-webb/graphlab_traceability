...
class Producer(threading.Thread):
    def __init__(self, name="", qn=20, t=1.0, loop=10, recipe=None, direct=''):
    ...
    def produce(self, t=1.0, idn=0):
        ...
        # description = ''
        r = [self.name, idn, 'nt', ts, te, qs, description, self.direct]
...
class Transporter(threading.Thread):
    def __init__(self, name, a, b, loop, t=1.0):
        ...
        self.JudgeCount = ''
        if 'c' in self.b.name:
            self.JudgeCount = self.b.buyNum
        elif 'p' in self.b.name:
            self.JudgeCount = self.b.recipe[self.a.name]
        else:
            print 'name参数不符合规定！'
    ...
    def trans(self, num=1, t=1.0):
        ...
        if qa.qsize() >= self.JudgeCount and qb.full() == False:   # 这里应该是 and!!!!!!!!!!!!!!!!!!!!!!!!!1
            qs = []
            for k in range(self.JudgeCount):
                qt = qa.get()
                qs.append(qt)
            ts = self.getTime()
            sleep(t)
...
class Consumer(threading.Thread):
    def __init__(self, name="", qn=20, t=1.0, loop=10, buyNum=2,direct=''):
        threading.Thread.__init__(self)
        self.name = name
        self.buyNum = buyNum    # 购买产品数量
        self.t = t
        self.f = open(name, 'w')        #
        self.loop = loop
        self.startime = settings.STARTIME
    ...
    def consume(self, t=1.0,idn=0):
        qs = []
        # 商品不足,购买失败
        if self.sin.qsize() < self.buyNum:
            sleep(self.Sleeptime)
            print self.name, idn
            return 0
...
class BSModel():
    ...
    def showBSgraph(self, pointName, idList):
        testShow = graphShow(pointName, idList)
        testShow.clearShowfile()
        testShow.createShowfile()
        testShow.createCSV(testShow.path,testShow.p)    # 'p4', [1]
        testShow.showPath(testShow.highlight)
        pass
    pass

    def showBPath(self, pointName, idList):
        testShow = graphBackShow(pointName, idList)
        testShow.clearShowfile()
        testShow.createShowfile()
...
if __name__ == '__main__':
    ...
    bs.CreateModel()
    bs.runModel()
    # bs.showBSgraph('c0', range(2))       # 可以对任一生产者的任一产品进行溯源
